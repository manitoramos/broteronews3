...
<meta http-equiv="content-type" content="text/html;charset=UTF-8" />
	<head> 
	<title>Website.com - jackpot? ez right</title> 
	...
	<body> 
		...
		<div id="container">
			...
			<div id="content">
				...
				<div class="col-lg-5 affix-right">
					<div id="affix-right" class="border-left"> 
						<div class="col-xs-24 chat">
			...
			<div class="site faq col-xs-24 col-sm-18 col-md-14 col-lg-12">
				...
				<br/>
				- By adding "Website.com" to your steam name and re-logging the site, you get +6% extra on your winnings!
				<br/>
				...
				<br/>
				<br/>
				Questions and Inquires can be sent to: support@Website.com
				<br/>
				<br/> 
			...
			<div class="site rules col-xs-24 col-sm-18 col-md-14 col-lg-12"> 
				...
				<br/>
				<br/>
				You must the age of 18+ (21+ in some places) or older to use our website. By using this website you agree to these terms and conditions, you warrant that you are at least 18/21 years of age. Website.com has the right to request identification of age to prevent under-aged gambling while temporarily disabiling your account. 
				<h4>Disclaimer:</h4>
				The use of our service is provided "AS IS" or "AS AVAILABLE". No guarantees are provided for ANY activity done on this site.
				<br/>
				We reserve the rights to have anything on Website or affiliated changed, whenever, without consent.
				<br/>
				<h4>License of service usage:</h4> 
				Website and/or its owners own the intellectual property rights and originality that is published on Website (in any form). Subject to the license below, all these intellectual property rights are reserved. 
				...
				<h4>Valuation:</h4>
				Website items are valued from SteamAnalyst's database backed up with the Steam Market. Prices are subjected to change any time without notice. 
				<br/> 
				All deposits are final after the user confirms them on the website, there will be no refunds, since the user agrees to enter the jackpot. Agrees he is liable to losinghis skins to another user and agrees Website is not liable for losses of any kind. 
				...
				<h4>Skin Deposits:</h4> 
				Website.com only allows items from game “Counter Strike: Global Offensive” (non-affiliated). These skins MUST be your own.Items from different games will be declined. 
				<h4>Variation:</h4>
				...
				<a class="close" data-page="rules"></a> 
			</div>
		</div> 
		<script id="user-room-panel-template" type="text/x-handlebars-template"> 
			<div class="ico"></div>
				<ul>
					<li>
						You've deposited - 
						<span>{{count_user_items}}</span>
						(of <span>{{room.max_user_items}}</span>)
						item(s).
					</li> 
					<li>
						Probably to win 
						<span>{{me.percent}}%</span>
						. Min bet 
						<span>${{room.min_user_items_value}}</span>
					</li> 
				</ul> 
		</script>
		<script id="room-template" type="text/x-handlebars-template">
			<div id="r{{id}}" class="room active" data-id="{{id}}">
			</div> 
		</script> 
		<script id="room-user-template" type="text/x-handlebars-template"> 
			<div class="item">
				<div class="row u{{openid}}" data-openid="{{openid}}" data-count-additions="{{count_additions}}">
					...
					<div class="desc"> 
						...
						<span class="right" onclick="system.items.sort('.i{{openid}}', this)">
							<span class="si">
								User Items
							</span>
							...
							<span class="winner hide">
								0
							</span> 
						</span> 
						<span class="room-price hide"></span>
						<span class="number hide">
							{{sort}}
						</span> 
					</div> 
		...
		<script id="item-template" type="text/x-handlebars-template">
			<div class="item i{{openid}}"> 
				<div class="row"> 
					<div class="col-lg-11 col-lg-offset-1 img"> 
						<img src="https://steamcommunity-a.akamaihd.net/economy/image/class/730/{{classid}}/{{width}}fx{{height}}f"> 
					</div>
		...
		<script id="chat-msg-template" type="text/x-handlebars-template"> 
			<div class="item{{#if pull_right}} in{{/if}} item-visible msg{{msg_id}}{{#if user.is_admin}} is_admin{{/if}}">
				{{#if user.is_admin}} 
					<div class="image">
						<span>
							<img alt="{{user.steam_nickname}}" src="{{user.steam_avatar}}">
						</span>
					</div>
				{{else}} 
					<div class="image">
						<a href="{{user.steam_profileurl}}" target="_blank">
							<img alt="{{user.steam_nickname}}" src="{{user.steam_avatar}}">
						</a>
					</div> 
				{{/if}}
				<div class="text">
					<div class="heading">
						...
						{{else}}
							<a href="{{#if user.is_admin}}#{{else}}{{user.steam_profileurl}}{{/if}}" target="_blank" class="username">
								{{user.steam_nickname}}
							</a> 
						{{/if}}
						{{#if admin_logged}} 
							<span onclick="system.admin.banUser('{{user.openid}}')" class="btn btn-xs btn-danger">
								B
							</span>
							<span onclick="system.admin.unbanUser('{{user.openid}}')" class="btn btn-xs btn-primary">
								U
							</span> 
							<span onclick="system.admin.deleteMsg('{{msg_id}}')" class="btn btn-xs btn-warning">
								D
							</span>
						{{/if}} 
					</div>
		...
		<script id="winner-row-template" type="text/x-handlebars-template"> 
			<tr class="{{#if is_top}} top{{/if}}">
				...
				<td class="text-left"> 
					<a rel="nofollow" target="_blank" href="{{user.steam_profileurl}}" class="img hidden-xs">
						<img src="{{user.steam_avatarmedium}}">
					</a>
					<a rel="nofollow" target="_blank" href="{{user.steam_profileurl}}" class="name">
						{{user.steam_nickname}}
					</a>
				</td>
				<td class="hidden-xs">{{total_wins}}</td> <td class="hidden-xs">
					${{total_cash}}
				</td>
				<td class="hidden-xs">
					${{total_won}}
			...
			<div class="modal fade" id="lobbyAcceptModal" tabindex="-1" role="dialog" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form class="form-horizontal">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
								<h4 class="modal-title">Deposit received</h4>
							</div>
							<div class="modal-body">
								<span class="line1">
									You deposited 
									<span class='items'></span> 
									skins and they were evaluated at 
									<span class='price'></span>
								</span>
								<span class="line2">You have 
								...
								<div class="form-group">
									<div class="col-sm-offset-2 col-sm-10">
										<div class="checkbox"> 
											<label id="accept-label"> 
												<input type="checkbox" id="accept-checkbox" value="1"> 
													I agree with the ToS, accept the value of the skins that are beeing deposited and confirm that I'm at least 18 years old. 
											</label> 
										</div> 
									</div> 
								</div> 
							...
							<div class="modal-footer"> 
								<button type="button" class="btn btn-primary btn-accept">
									Accept and Enter the Queue
								</button>
								<button type="button" class="btn btn-danger btn-deceline">
